...
class RMSNorm(torch.nn.Module):
    def __init__(self, dim: int, eps: float = EPSILON_STABILITY):
        super().__init__()
        self.eps = eps
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, context_length):
        ...
        # Note: context_length variation will cause recompilation but not graph breaks.
        # Training now samples from a small fixed bucket set per epoch to keep the number of compiled graphs bounded.
