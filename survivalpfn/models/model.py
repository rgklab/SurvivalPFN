...
def normalize_data(data, eval_pos):
    ...
    mask = ~torch.isnan(X)
    mean = maskmean(X, mask, dim=0)
    std = maskstd(X, mask, dim=0) + EPSILON_STABILITY
    data = (data - mean) / std
    return data
...
class TabDPTLongContextModel(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(
            nn.Linear(ninp, nhid, bias=False),
            nn.GELU(),
            nn.Linear(nhid, n_out + nbins, bias=False),
        )
    ...
    def forward(
        ...
        x_src = clip_outliers(
            x_src, -1 if self.training else context_length, n_sigma=10
        )
        ...
        for layer_idx, layer in enumerate(self.transformer_encoder):
            if layer_idx in [0, 1, 3, 6, 9]:
                log_act_norms[f"layer_{layer_idx}"] = torch.norm(src, dim=-1).mean()
            src = layer(src, context_length)

        pred = self.head(src)

        # NOTE: some trick in the previous TabDPT paper, discard this should not affect the performance
    ...
    def load(cls, model_state, config):
        ...
        dropout = getattr(getattr(config, "training", None), "dropout", 0.0)
